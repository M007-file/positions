...
function obtain($url, $searchfor, $regex, $offset, $offsetend){
    $browsers = array("Internet Explorer","Google Chrome","Mozilla Firefox","Safari","Opera","Konqueror","Lynx","Microsoft Edge","SeaMonkey","Vivaldi","Yandex");
    shuffle($browsers);
    $curl_handle=curl_init();
    curl_setopt($curl_handle, CURLOPT_URL,$url);
    curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
    curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($curl_handle, CURLOPT_USERAGENT, $browsers[0]." ".rand(967,4650).'.'.rand(167,950));
  ...
  die("Connection failed: " . mysqli_connect_error());
    ...
    if (mysqli_num_rows($resultsql1)>0) {
        while($row = mysqli_fetch_assoc($resultsql1)) { //URLs with the data in sources table at DB
            echo $row["Website"]."<br />\r\n";
            $urlarr = obtain($row["Website"],"divFixedHideMobile",$re,6,-1); //"href="https://www.domain.com"" - searching for HTML element class name
            $DB1 = intodb($conn,$urlarr,"urls", "Website");
            $katalogarr = obtain($row["Website"],"divStrankovacH",$reedb,6,-1); //"href="https://www.edb.cz/katalog-firem/elektro-a-pocitace/?p=2"" - searching for HTML element class name
            $DB2 = intodb($conn,$katalogarr,"sources", "Website");
            $mailyarr = obtain($row["Website"],"mailto",$remail,13,-1); //"href="mailto:firstname.surname@domain.com"" - in general            
            $DB3 = intodb($conn,$mailyarr,"emaily", "email");
        }
    } else { //loading of initial source URLs without data in sources table at DB
        $urlarr = obtain($masterurl,"divLevy",$re,6,-1); //"href="https://www.domain.com"" - searching for HTML element class name;
        $DB1 = intodb($conn,$urlarr,"sources", "Website");
        $urlarr = obtain($masterurl,"divStredni",$re,6,-1); //"href="https://www.domain.com"" - searching for HTML element class name;
